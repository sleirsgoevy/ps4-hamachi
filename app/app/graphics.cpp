...
bool Scene2D::InitFreeType(void)
{
	...
	if (rc < 0)
	{
		printf("Failed to load freetype: %s\n", strerror(errno));
		return false;
	}
	...
	if (rc != 0)
	{
		printf("Failed to initialize freetype: %s\n", strerror(errno));
		return false;
	}
#endif
	return true;
	...
	for(int n = 0; n < strlen(txt); n++)
	{
        ...
        for (int yPos = 0; yPos < slot->bitmap.rows; yPos++)
        {
            for (int xPos = 0; xPos < slot->bitmap.width; xPos++)
            {
                ...
                uint8_t pixel = slot->bitmap.buffer[(yPos * slot->bitmap.width) + xPos];
				...
				uint8_t r = (pixel * fgColor.r + (255 - pixel) * bgColor.r) / 255;
				uint8_t g = (pixel * fgColor.g + (255 - pixel) * bgColor.g) / 255;
				uint8_t b = (pixel * fgColor.b + (255 - pixel) * bgColor.b) / 255;
...
void Scene2D::GetTextDimensions(char *txt, FT_Face face, int& width, int& height)
{
	int rc;
	width = 0;
	height = 0;
	...
	for(int n = 0; n < strlen(txt); n++)
	{
        ...
        if(xOffset >= width)
            width = xOffset;
        if(yOffset + slot->bitmap.width >= height)
            height = yOffset + slot->bitmap.width * 2;
    }
}
#endif
